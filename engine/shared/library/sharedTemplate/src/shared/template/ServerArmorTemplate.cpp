...
//@BEGIN TFD INIT
        : TpfTemplate(filename), m_specialProtectionLoaded(false), m_specialProtectionAppend(false)
//@END TFD INIT
{
...
int ServerArmorTemplate::getListLength(const char *name) const {
    if (strcmp(name, "encumbrance") == 0) {
        return sizeof(m_encumbrance) / sizeof(CompilerIntegerParam);
    } else if (strcmp(name, "specialProtection") == 0) {
        ...
        switch (runtimeCrc(paramName)) {
            ...
            case constcrc("encumbrance"): {
                ...
                DEBUG_WARNING(listCount != 3,
                              ("Template %s: read array size of %d for array \"encumbrance\" of size 3, reading values anyway", file.getFileName(), listCount));
                int j;
...
ServerArmorTemplate::_SpecialProtection::getCompilerIntegerParam(const char *name, bool deepCheck, int index) {
    switch (runtimeCrc(name)) {
        case constcrc("type"): {
            if (index == 0) {
                if (deepCheck && !isParamLoaded(name, false, 0)) {
...
ServerArmorTemplate::_SpecialProtection::getDynamicVariableParam(const char *name, bool deepCheck, int index) {
    return TpfTemplate::getDynamicVariableParam(name, deepCheck, index);
...
TriggerVolumeParam *
ServerArmorTemplate::_SpecialProtection::getTriggerVolumeParam(const char *name, bool deepCheck, int index) {
    return TpfTemplate::getTriggerVolumeParam(name, deepCheck, index);
