...
void ConnectionServerConnection::onReceive (const Archive::ByteStream & message)
{
	...
	else if (m.isType("ChatOnChangeFriendStatus"))
	{
		ChatOnChangeFriendStatus chat(ri);

		CommunityManager::handleMessage(chat);
	}
	else if (m.isType("ChatOnGetFriendsList"))
	{
		ChatOnGetFriendsList chat(ri);

		CommunityManager::handleMessage(chat);
	}
	else if (m.isType("ChatOnChangeIgnoreStatus"))
	{
		ChatOnChangeIgnoreStatus chat(ri);

		CommunityManager::handleMessage(chat);
	}
	else if (m.isType("ChatOnGetIgnoreList"))
	{
		ChatOnGetIgnoreList chat(ri);

		CommunityManager::handleMessage(chat);
	}
	else if (m.isType("ChatStatisticsCS"))
	{
	...
	else if (m.isType("RequestLoadPackedHouses"))
	{
	...
	else if (m.isType("RequestLoadCTSBank"))
	{
		GenericValueTypeMessage<NetworkId> characterId(ri);
	...
	else if (m.isType("ApplyTransferData"))
	{
	...
	else if (m.isType("ChatEnterRoomValidationRequest"))
	{
		Archive::ReadIterator readIterator = message.begin();
	...
	else if (m.isType("ChatQueryRoomValidationRequest"))
	{
		Archive::ReadIterator readIterator = message.begin();
	...
	else if (m.isType("PlayerDroppedFromGameServerCrash"))
	{
		Archive::ReadIterator readIterator = message.begin();
