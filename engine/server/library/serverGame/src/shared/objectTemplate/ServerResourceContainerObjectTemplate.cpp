...
UNREF(testData);
	...
	const ServerResourceContainerObjectTemplate * base = NULL;
	if (m_baseData != NULL)
	{
	...
	if (!m_maxResources.isLoaded())
	{
		...
		{
			DEBUG_FATAL(base == NULL, ("Template parameter maxResources has not been defined in template %s!", DataResource::getName()));
